...
class Expenses extends React.Component {
  ...
  componentDidMount() {
    ...
    axios
      .get(
        "https://hidden-everglades-59214.herokuapp.com/app/v1/products/?sort=date:desc",
  ...
  componentDidUpdate() {
    if (this.state.yearlySelected === "all") {
      ...
      this.state.yearlySelected != null &&
      this.state.yearlySelected.length === 4 &&
      !this.state.monthlyDisplay
    ) {
      let dateFrom = new Date(
        `${this.state.yearlySelected}-01-01 00:00:00.000`
      ).getTime();
      let dateTo = new Date(
        `${this.state.yearlySelected}-12-31 23:59:59.000`
      ).getTime();
      console.log(dateFrom + " " + dateTo);
      axios
        .get(
          `https://hidden-everglades-59214.herokuapp.com/app/v1/products/?date_from=${dateFrom}&date_to=${dateTo}`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("jwt")}`,
            },
      ...
      this.state.monthlySelected != null &&
      this.state.yearlySelected != null &&
      this.state.monthlyDisplay
    ) {
      console.log("Vleze");
      ...
      let dateFrom = new Date(
        `${this.state.yearlySelected}-${monthNum}-01 00:00:00.000`
      ).getTime();
      console.log(monthNum + "Month is");
      let dateTo = new Date(
        `${this.state.yearlySelected}-${monthNum}-31 23:59:59.000`
      ).getTime();
      console.log(dateFrom + " " + dateTo);
      console.log(monthNum + "Month is");
      axios
  ...
  render() {
    let yearly = (
      <select
        ...
        onChange={this.yearlySelectHandler}
      >
    ...
    let monthly = (
      <select
        name='month-select'
        className='month-select'
        onChange={this.monthlySelectHandler}
      >
        <option defaultChecked value='months'>
          Month
        </option>
    ...
    return (
      <React.Fragment>
        ...
        <div className='main-select-div'>
          ...
          <div className='expenses-div'>
            <Link to='#'>
              <button
                className={
                  !this.state.active ? "active-btn select-btn" : "select-btn"
                }
                onClick={this.yearlySelect}
              >
                Yearly
              </button>
            </Link>
            ...
            <div className='select-div'>
              {this.state.monthlyDisplay ? (
                <label htmlFor='month-select'>Choose month and year:</label>
              ) : (
        ...
        <div className='transparent-div'>
          <p>
            Total spent:<span>{this.props.totalPrice}</span> den.
          </p>
        </div>
        {this.state.loading && (
          <div style={{ color: "grey", fontSize: "50px", textAlign: "center" }}>
            LOADING...
          </div>
        )}
      </React.Fragment>
